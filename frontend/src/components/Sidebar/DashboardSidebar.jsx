...
export default function DashboardSidebar({ 
  ...
  const navItems = [
    ...
    { id: 'companies', label: t('nav.companies', 'Verified Companies'), icon: Building2, badge: '40+' },
  ...
  return (
    <aside 
      className={`fixed top-0 left-0 bottom-0 z-40 bg-[#0a0f1d]/95 backdrop-blur-2xl border-r border-slate-800/80 flex flex-col justify-between transition-all duration-300 select-none ${
        isCollapsed ? 'w-20' : 'w-64'
      }`}
    >
      {/* Top Header & Brand */}
      <div>
        <div className={`p-4 flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} border-b border-slate-800/60`}>
          {!isCollapsed ? (
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-2xl bg-gradient-to-tr from-cyan-500 to-blue-600 p-[1px] flex items-center justify-center shadow-md shadow-cyan-500/20">
                <div className="w-full h-full bg-[#0b101d] rounded-[15px] flex items-center justify-center">
                  <ShieldCheck className="w-5 h-5 text-cyan-400" />
                </div>
              </div>
              <div>
                <span className="text-base font-extrabold tracking-tight text-white">Hire<span className="text-cyan-400">Shield</span></span>
        ...
        {/* Un-collapse button for collapsed state */}
        {isCollapsed && (
        ...
        {!isCollapsed ? (
          ...
          <div className="flex flex-col items-center gap-2">
            {user?.avatar_url ? (
              ...
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-cyan-600 to-blue-600 flex items-center justify-center text-white text-xs font-bold">
                {getInitials(user?.name)}
              </div>
            )}
            <button
